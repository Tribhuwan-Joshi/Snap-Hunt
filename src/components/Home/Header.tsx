...
function CharacterMobile() {
  return (
    <div className="mobile-char md:hidden top-12 flex fixed left-[50%] translate-x-[-50%] origin-center mx-auto w-[80%] text-white h-14 bg-gray-700/70 ">
      {characters.map((i) => {
        return (
          <div
            key={i.alt}
            className=" flex-1   duration-500 font-mono  items-center transistion ease-in-out flex  "
          >
            {" "}
            <img
              src={i.url}
              className="w-[10vw] min-w-[40px] h-auto"
              title={i.alt}
              alt={i.alt}
            />{" "}
            <div className="text-xs">{i.alt}</div>{" "}
          </div>
        );
      })}
    </div>
...
function Hamburger({ handleBarClick }: { handleBarClick: () => void }) {
  return (
    <img
      src={hamburger}
      className="cursor-pointer hamburger md:hidden lg:hidden"
      alt="show"
...
function Logo() {
  return (
    <h1 className="text-[28px] md:text-[40px] lg:text-[48px] md:ml-2">
      Snap<span className="text-red-500">Hunt</span>
    </h1>
  );
}
function Header() {
